      ...
      <div class="space-y-2">
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-semibold cursor-pointer" @click="startEditing(product, 'name')">
            ...
            <input v-else v-model="product.name" @blur="saveEdit(product, index)" class="w-full border rounded px-2"
              autofocus />
          </h3>
          <div v-if="product.stock === 0" class="bg-black text-white px-2 py-1 rounded-full text-xs ml-3">
            Out of Stock
          </div>
          <div v-else class="text-white px-2 py-1 rounded-full text-xs ml-3" :class="{
        ...
        <div class="flex items-center">
          ...
          <span class="ml-2 cursor-pointer" @click="startEditing(product, 'price')">
            ...
            <div v-else class="relative">
              <input v-model.number="product.price" type="number" step="0.01" min="0" @blur="saveEdit(product, index)"
                class="w-20 border rounded px-2" :class="{ 'border-red-500': priceErrorIndex === index }" />
            </div>
          </span>
        </div>
        ...
        <div class="flex items-center">
          ...
          <span class="ml-2 flex items-center">
            ...
            <span v-if="product.showStock" class="cursor-pointer px-1" @click="startEditing(product, 'stock')">
              <span class="rounded px-2 bg-blue-100" v-if="!product.editing">{{
                product.stock
              }}</span>
              <div v-else class="relative">
                <input v-model.number="product.stock" type="number" min="0" @blur="saveEdit(product, index)"
                  class="w-20 border rounded px-2" :class="{ 'border-red-500': stockErrorIndex === index }" />
              </div>
            </span>
          </span>
        </div>
        ...
        <div class="flex items-center">
          ...
          <span class="ml-2 cursor-pointer" @click="startEditing(product, 'category')">
            ...
            <select v-else v-model="product.category" @change="saveEdit(product, index)" class="border rounded px-2">
        ...
        <div class="flex justify-between items-center pt-4 border-t mt-4">
          <button @click="toggleStockVisibility(product)" class="text-sm px-3 py-1 rounded" :class="{
          ...
          <button v-if="product.editing" class="text-sm px-3 py-1 rounded bg-red-100 text-gray-600"
